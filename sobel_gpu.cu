...
sobel_filtered_pixel(float *s, int i, int j, int ncols, int nrows, float *gx, float *gy)
{
   ...
   for (int ki = -1; ki <= 1; ki++)
   {
      for (int kj = -1; kj <= 1; kj++)
      {
         ...
         {
            float val = s[ni * ncols + nj];
            Gx += val * gx[(ki + 1) * 3 + (kj + 1)];
            Gy += val * gy[(ki + 1) * 3 + (kj + 1)];
         }
      }
   }
                 ...
                 float *gx, float *gy) // gx and gy are stencil weights for the sobel filter
{
   ...
   {
      int i = id / ncols; // Row index
      int j = id % ncols; // Column index

      if (i > 0 && i < nrows - 1 && j > 0 && j < ncols - 1)
      {
         d[id] = sobel_filtered_pixel(s, i, j, ncols, nrows, gx, gy);
      }
   }
}
